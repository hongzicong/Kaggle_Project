...
rfc = RandomForestRegressor(n_estimators=1000, max_depth=20, oob_score = True, n_jobs=-1, random_state = RANDOM_SEED)
...
predictions = [round(value) for value in rfc_y_predict]
...
pd.DataFrame(result,columns=['Predicted'],index=list(range(1,len(result) + 1))).to_csv('../../dm2019springproj2/result_rfc.csv')
